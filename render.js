...
function render(content) {
  ...
  // Resume button: only show it once an actual CV file has been uploaded
  // in the CMS. Otherwise hide it rather than pointing it at Contact.
  ...
  // LinkedIn: hidden until a URL is added in the CMS, so it can be
  // switched on later with no code changes.
  const linkedinDesktop = document.getElementById('linkedin-link');
  const linkedinMobile = document.getElementById('mobile-linkedin-link');
  if (site.linkedin_url) {
    // Desktop icon defers to its "hidden md:flex" classes once the inline
    // display:none is cleared; the mobile menu link is always flex, so it
    // just needs to be shown.
    if (linkedinDesktop) { linkedinDesktop.setAttribute('href', site.linkedin_url); linkedinDesktop.style.display = ''; }
    if (linkedinMobile) { linkedinMobile.setAttribute('href', site.linkedin_url); linkedinMobile.style.display = 'flex'; }
  } else {
    if (linkedinDesktop) linkedinDesktop.style.display = 'none';
    if (linkedinMobile) linkedinMobile.style.display = 'none';
  }
